...
def main(tiles_data_path, config_path, experiment):
	...
	for idx, participant_id in enumerate(top_participant_id_list[:]):
		...
		omsignal_data_df = load_sensor_data.read_omsignal(os.path.join(tiles_data_path, '3_preprocessed_data/omsignal/'), participant_id)
		
		# If we don't have fitbit data, no need to process it
		if omsignal_data_df is None or owl_in_one_df is None:
			print('%s has no owl-in-one or omsignal data' % participant_id)
			continue

		filter_class.filter_data(mgt_df=participant_mgt, owl_in_one_df=owl_in_one_df, omsignal_df=omsignal_data_df)
