...
def main(tiles_data_path, config_path, experiment):
    ...
    fitbit_norm_data_df = pd.read_csv(os.path.join(save_data_path, data_config.fitbit_sensor_dict['filter_path'].split('/')[-1], 'norm_data_ticc_cluster_days_' + str(data_config.fitbit_sensor_dict['ticc_cluster_days']) + '.csv.gz'), index_col=3)
    fitbit_dict_df = pd.read_csv(os.path.join(save_data_path, data_config.fitbit_sensor_dict['filter_path'].split('/')[-1], 'dict_norm_data_cluster_days_' + str(data_config.fitbit_sensor_dict['ticc_cluster_days']) + '.csv.gz'), index_col=0)
