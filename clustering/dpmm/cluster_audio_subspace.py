...
def cluster_audio(data_df, data_config, iter=100):
    ...
    # process_col_list = ['F0_sma', 'duration', 'pcm_loudness_sma', 'logHNR_sma',
    ...
    for process_col in process_col_list:
        ...
        if data_config.audio_sensor_dict['cluster_method'] == 'collapsed_gibbs':
            ...
            # Intialize prior
            covar_scale = np.var(np.array(col_data_df))
            # covar_scale = np.median(LA.eigvals(np.cov(np.array(col_data_df).T)))
            mu_scale = np.amax(np.array(col_data_df)) - covar_scale
            ...
            dpgmm = mixture.BayesianGaussianMixture(n_components=20, max_iter=500, covariance_type='full').fit(
                np.array(col_data_df))
            cluster_id = dpgmm.predict(np.array(col_data_df))
...
def main(tiles_data_path, config_path, experiment):
    ...
    # 0.1, 1.1, 0 - 20, 20 - 40, 40 - 60, 60 - 80 80 - 100 100 - 120 120 - 140 140 - 160
    # 1, 1.1,   0 - 20
