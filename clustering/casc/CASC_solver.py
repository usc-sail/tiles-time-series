...
class CASCSolver:
    ...
    def solveWithInitialization(self, clustered_points, useMotif):
        ...
        for iters in range(self.maxIters):
            ...
            clustered_points = updateClusters(LLE_all_points_clusters, switch_penalty=beta)
            clustered_points = [i.astype(int) for i in clustered_points]
            if useMotif:
                clustered_points, motifs, rankedMotifs = PerformAssignment(clustered_points, LLE_all_points_clusters, self)
                
                for m, score in rankedMotifs:
                    print("%s ---> %s, %s" % (m, len(motifs[m]), score))
                    
            before_zero = clustered_points.copy()
            self.assignToZeroClusters(clustered_points, old_computed_cov, computed_cov, cluster_mean_stacked_info)
    ...
    def solveForClusters(self, clust_indices, cluster_mean_stacked_info,
        ...
        for cluster in range(K):
            if cluster_lens[cluster] != 0:
                cluster_data = np.take(self.complete_data, clust_indices[cluster], axis=0)
                cluster_mean_stacked_info[cluster] = np.mean(cluster_data, axis=0)
                ...
                optRes[cluster] = self.pool.apply_async(solver, (1000, 1e-6, 1e-6, False,))
