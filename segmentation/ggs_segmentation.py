...
def main(tiles_data_path, config_path, experiement):
    ...
    ###########################################################
    if os.path.exists(os.path.join(data_config.fitbit_sensor_dict['segmentation_path'], 'participant_id.csv.gz')) is True:
        top_participant_id_df = pd.read_csv(os.path.join(data_config.fitbit_sensor_dict['segmentation_path'], 'participant_id.csv.gz'), index_col=0, compression='gzip')
    else:
