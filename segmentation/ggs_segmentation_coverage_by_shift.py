...
def main(tiles_data_path, config_path, experiement):
	...
	for idx, participant_id in enumerate(top_participant_id_list):
		...
		unique_cluster_list = list(set(all_df['cluster']))
		...
		for file_name in file_list:
			...
			cluster_data_df = pd.read_csv(os.path.join(save_participant_folder, file_name), index_col=0)
			for time_str in list(cluster_data_df.index):
				row_df = cluster_data_df.loc[time_str, :].to_frame().T

				start_str, end_str = row_df['start'][0], row_df['end'][0]
				owl_in_one_seg_df =owl_in_one_df[start_str:end_str]

				if len(owl_in_one_seg_df) == 0:
					continue

				for col in unique_loc_list:
					ratio = len(owl_in_one_seg_df.loc[owl_in_one_seg_df[col] == 1]) / len(owl_in_one_seg_df)
					row_df[col] = ratio

				ratio_df = ratio_df.append(row_df)
			...
			ratio_df.to_csv(os.path.join(save_clustering_participant_folder, file_name.split('_')[0] + '_ratio.csv.gz'), compression='gzip')
