...
class Segmentation(object):
	...
	def segment_workday_data(self, data_dict, threshold=0):
		...
		for key_str in key_list:
			...
			norm_data = np.array(data_df).astype(float)
			...
			# Find breakpoints with lambda = segmentation_lamb
			###########################################################
			if not os.path.exists(os.path.join(save_participant_folder, key_str + '.csv.gz')):
				...
				bps_start_str = [list(data_df.index)[i] for i in bps_final[:-1]]
				bps_end_str = [list(data_df.index)[i] for i in bps_final[1:]]
				bps_str = [list(data_df.index)[i] for i in bps_final]

				bps_df = pd.DataFrame(bps_start_str, columns=['start'], index=bps_start_str)
				bps_df['end'] = bps_end_str

				###########################################################
				# Save data
				###########################################################
				bps_df.to_csv(os.path.join(save_participant_folder, key_str + '.csv.gz'), compression='gzip')


	def segment_shift_data(self, data_dict, threshold=0):
		...
		for key_str in key_list:
			...
			if not os.path.exists(os.path.join(save_participant_folder, key_str + '.csv.gz')):
				...
				# Save data
				###########################################################
				bps_df.to_csv(os.path.join(save_participant_folder, key_str + '_shift.csv.gz'), compression='gzip')
