...
def process_sleep_realizd(data_config, data_df, days_at_work_df, igtb_df, participant_id):
    days = (pd.to_datetime(data_df.index[-1]).date() - pd.to_datetime(data_df.index[0]).date()).days
    
    process_df = pd.DataFrame()
    
    if len(data_df) < 1000 and len(days_at_work_df) < 10:
        return None
...
def main(tiles_data_path, config_path, experiment):
    ...
    for idx, participant_id in enumerate(top_participant_id_list[:]):
        ...
        participant_df = process_sleep_realizd(data_config, realizd_raw_df, days_at_work_df, igtb_df, participant_id)
