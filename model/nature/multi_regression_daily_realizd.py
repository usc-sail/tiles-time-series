...
def main(tiles_data_path, config_path, experiment):
	...
	if os.path.exists(os.path.join(data_config.phone_usage_path, 'daily_summary.csv.gz')) is True:
		...
		data_cols = ['Total Usage Time', 'Mean Session Length', 'Session Frequency', 'Mean Inter-session Time', 'Session Frequency (<1min)', 'Session Frequency (>1min)']

		X = work_day_df[data_cols]
		for col in ana_igtb_cols:
			y = work_day_df[ana_igtb_dict[col]]
