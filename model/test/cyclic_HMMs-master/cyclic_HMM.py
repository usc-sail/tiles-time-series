...
class emissionDistribution(object):
    ...
    def initialize_using_samples(self, samples):  
        ...
        random_subset_of_samples = random.sample(range(len(samples)), max(1, min(2000, int(len(samples) / 2))))
...
class cyclic_HMM(object): 
    ...
    def get_state_duration_params(self, j, transition_matrix_counts = None): 
        ...
        durations = np.array(list(durations_to_probabilities.keys()))
        ps = np.array(list(durations_to_probabilities.values()))
