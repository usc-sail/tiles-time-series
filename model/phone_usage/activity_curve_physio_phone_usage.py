...
def clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array, cluster_type='realizd'):
    num_point_per_day = chi_data_config.num_point_per_day
    ...
    if len(final_data_df) > 1440 * 4:
        if cluster_type == 'realizd':
            ...
            model, cluster_list, min_array, max_array = cluster_data(final_data_df[['HeartRatePPG', 'StepCount']],
                                                                     chi_data_config, iter=300)
        return model, cluster_list, min_array, max_array
    else:
        return None, None, None, None
...
def cal_shuffle_dist(agg, sliding, num_point_per_day, realizd_df, fitbit_df, agg_daily_array,
    ...
    for dates in range(dates_range):
        ...
        for shuffle_idx in range(100):
            ...
            agg_realizd_first_array = (agg_first_array[:, :2] - min_realizd_array) / (max_realizd_array - min_realizd_array)
            agg_realizd_second_array = (agg_second_array[:, :2] - min_realizd_array) / (max_realizd_array - min_realizd_array)
            agg_physio_first_array = (agg_first_array[:, 2:] - min_physio_array) / (max_physio_array - min_physio_array)
            agg_physio_second_array = (agg_second_array[:, 2:] - min_physio_array) / (max_physio_array - min_physio_array)
    
            first_realizd_cluster_array = realizd_model.predict(agg_realizd_first_array)
            second_realizd_cluster_array = realizd_model.predict(agg_realizd_second_array)
            ...
            for i in range(48):
                ...
                for cluster_id in list(counter_dict.keys()):
                    if cluster_id in realizd_cluster_list:
                        second_realizd_distribution[i, realizd_cluster_list.index(cluster_id)] = counter_dict[cluster_id]
    
                epsilon = 0.00001
    
                first_pdf, second_pdf = first_realizd_distribution[i, :], second_realizd_distribution[i, :]
                ...
                for cluster_id in list(counter_dict.keys()):
                    if cluster_id in physio_cluster_list:
                        second_physio_distribution[i, physio_cluster_list.index(cluster_id)] = counter_dict[cluster_id]

                first_pdf, second_pdf = first_physio_distribution[i, :], second_physio_distribution[i, :]
                first_pdf, second_pdf = first_pdf / np.sum(first_pdf), second_pdf / np.sum(second_pdf)
                kl_dist = entropy(first_pdf + epsilon, second_pdf + epsilon) + entropy(second_pdf + epsilon, first_pdf + epsilon)
                kl_dist = kl_dist / 2
            ...
            dist_dict[dates][shuffle_idx]['realizd'] = {}
            dist_dict[dates][shuffle_idx]['realizd']['first'] = first_realizd_cluster_array
            dist_dict[dates][shuffle_idx]['realizd']['second'] = second_realizd_cluster_array
            dist_dict[dates][shuffle_idx]['realizd']['dist'] = kl_realizd_dist_array

            dist_dict[dates][shuffle_idx]['physio'] = {}
            dist_dict[dates][shuffle_idx]['physio']['first'] = first_physio_cluster_array
            dist_dict[dates][shuffle_idx]['physio']['second'] = second_physio_cluster_array
            dist_dict[dates][shuffle_idx]['physio']['dist'] = kl_physio_dist_array
...
def cal_regular_dist(agg, sliding, num_point_per_day, realizd_df, fitbit_df, agg_daily_array,
    ...
    for dates in range(dates_range):
        ...
        agg_first_realizd_array = (agg_first_array[:, :2] - min_realizd_array) / (max_realizd_array - min_realizd_array)
        agg_second_realizd_array = (agg_second_array[:, :2] - min_realizd_array) / (max_realizd_array - min_realizd_array)

        agg_first_physio_array = (agg_first_array[:, 2:] - min_physio_array) / (max_physio_array - min_physio_array)
        agg_second_physio_array = (agg_second_array[:, 2:] - min_physio_array) / (max_physio_array - min_physio_array)
        
        first_realizd_cluster_array = realizd_model.predict(agg_first_realizd_array)
        second_realizd_cluster_array = realizd_model.predict(agg_second_realizd_array)

        first_physio_cluster_array = physio_model.predict(agg_first_physio_array)
        second_physio_cluster_array = physio_model.predict(agg_second_physio_array)
        ...
        dist_dict[dates]['realizd']['first'] = first_realizd_cluster_array
        dist_dict[dates]['realizd']['second'] = second_realizd_cluster_array
        dist_dict[dates]['realizd']['dist'] = kl_realizd_dist_array

        dist_dict[dates]['physio'] = {}
        dist_dict[dates]['physio']['first'] = first_physio_cluster_array
        dist_dict[dates]['physio']['second'] = second_physio_cluster_array
        dist_dict[dates]['physio']['dist'] = kl_physio_dist_array
...
def main(tiles_data_path, config_path, experiment):
    ...
    for idx, participant_id in enumerate(top_participant_id_list[:]):
        ...
        # Clustering
        realizd_model, realizd_cluster_list, min_realizd_array, max_realizd_array = clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array, cluster_type='realizd')
        physio_model, physio_cluster_list, min_physio_array, max_physio_array = clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array, cluster_type='physio')
        ...
        output = open(os.path.join(chi_data_config.activity_curve_path, participant_id + '_combine.pkl'), 'wb')
