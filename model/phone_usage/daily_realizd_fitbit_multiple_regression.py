...
def main(tiles_data_path, config_path, experiment):
	...
	fitbit_col_list = ['step_count_mean', 'sleep_in_bed_minute',
					   'peak_ratio', 'cardio_ratio', # 'out_of_range_ratio',
					   'fat_burn_ratio'] # 'cardio_ratio'
	realizd_col_list = ['total_time', 'less_than_1min', 'above_1min', 'mean_time',
	...
	for compare_method in compare_method_list:
		...
		compare_cols = ['itp_igtb', 'irb_igtb', 'ocb_igtb', 'stai_igtb', 'pos_af_igtb', 'neg_af_igtb', 'Pain',
						'energy_fatigue']
		
		# all_col_list = realizd_analysis_col_list + fitbit_analysis_col_list + igtb_cols + add_cols
		all_col_list = realizd_analysis_col_list + fitbit_analysis_col_list + compare_cols
		...
		'''
		norm_df = mr_df.loc[:, realizd_analysis_col_list + fitbit_analysis_col_list]
		norm_df.loc[:, :] = (norm_df - norm_df.mean()) / norm_df.std()
		mr_df.loc[:, realizd_analysis_col_list + fitbit_analysis_col_list] = norm_df.loc[:, realizd_analysis_col_list + fitbit_analysis_col_list]
		
		norm_df = mr_df.loc[:, igtb_cols]
		norm_df.loc[:, :] = (norm_df - norm_df.mean()) / norm_df.std()
		mr_df.loc[:, igtb_cols] = norm_df.loc[:, igtb_cols]
		'''
		
		'''
		mr_df.loc[list(first_data_df.index), compare_method] = 0
		mr_df.loc[list(second_data_df.index), compare_method] = 1
		
		mr_df = (mr_df - mr_df.mean()) / mr_df.std()
		'''
		
		'''
		row_df = pd.DataFrame(index=[compare_method])
		for col in igtb_cols:
			tmp_df = mr_df[[col, compare_method] + realizd_analysis_col_list + fitbit_analysis_col_list].dropna()
			x = tmp_df[[compare_method] + realizd_analysis_col_list + fitbit_analysis_col_list]
			
			# tmp_df = mr_df[[col] + realizd_analysis_col_list + fitbit_analysis_col_list].dropna()
			# x = tmp_df[realizd_analysis_col_list + fitbit_analysis_col_list]
		...
		row_df['num'] = len(first_data_df[realizd_analysis_col_list + fitbit_analysis_col_list].dropna())
		for col in compare_cols:
			tmp_df = first_data_df[[col] + realizd_analysis_col_list + fitbit_analysis_col_list].dropna()
			x = tmp_df[realizd_analysis_col_list + fitbit_analysis_col_list]
			y = tmp_df[col]
			x = sm.add_constant(x)
			model = sm.OLS(y, x).fit()
		...
		row_df['num'] = len(second_data_df[realizd_analysis_col_list + fitbit_analysis_col_list].dropna())
		for col in compare_cols:
			tmp_df = second_data_df[[col] + realizd_analysis_col_list + fitbit_analysis_col_list].dropna()
			x = tmp_df[realizd_analysis_col_list + fitbit_analysis_col_list]
			y = tmp_df[col]
			x = sm.add_constant(x)
			model = sm.OLS(y, x).fit()
