...
def clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array):
    ...
    if len(final_data_df) > 1440 * 4:
        physio_model, physio_cluster_list, min_array, max_array = cluster_data(final_data_df[['HeartRatePPG', 'StepCount']], chi_data_config, iter=300)
...
def cal_shuffle_dist(agg, sliding, num_point_per_day, realizd_df, fitbit_df, agg_daily_array,
                     physio_model, physio_cluster_list, min_array, max_array):
    ...
    for dates in range(dates_range):
        ...
        for shuffle_idx in range(100):
            ...
            first_physio_cluster_array = physio_model.predict(agg_first_array)
            second_physio_cluster_array = physio_model.predict(agg_second_array)
            ...
            for i in range(48):
                ...
                kl_physio_dist = entropy(first_pdf + epsilon, second_pdf + epsilon) + entropy(second_pdf + epsilon, first_pdf + epsilon)
                kl_physio_dist = kl_physio_dist / 2
                kl_physio_dist_array[0, i] = kl_physio_dist

            dist_dict[dates][shuffle_idx] = {}
            dist_dict[dates][shuffle_idx]['first_physio'] = first_physio_cluster_array
            dist_dict[dates][shuffle_idx]['second_physio'] = second_physio_cluster_array
            dist_dict[dates][shuffle_idx]['physio_dist'] = kl_physio_dist_array
    
    return dist_dict


def cal_regular_dist(agg, sliding, num_point_per_day, realizd_df, fitbit_df,
                     agg_daily_array, physio_model, physio_cluster_list, min_array, max_array):
    dates_range = int(((pd.to_datetime(realizd_df.index[-1]) - pd.to_datetime(realizd_df.index[0])).days - agg) / sliding)
    ...
    for dates in range(dates_range):
        ...
        dist_dict[dates] = {}
        dist_dict[dates]['first_physio'] = first_physio_cluster_array
        dist_dict[dates]['second_physio'] = second_physio_cluster_array
        dist_dict[dates]['physio_dist'] = kl_physio_dist_array
...
def main(tiles_data_path, config_path, experiment):
    ...
    for idx, participant_id in enumerate(top_participant_id_list[60:100]):
        ...
        # Clustering
        physio_model, physio_cluster_list, min_array, max_array = clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array)
        
        if physio_model is None:
            continue
        ...
        output = open(os.path.join(chi_data_config.activity_curve_path, participant_id + '_physio.pkl'), 'wb')
