...
def clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array):
    ...
    if len(final_data_df) > 1440 * 4:
        model, cluster_list, min_array, max_array = cluster_data(final_data_df[['SecondsOnPhone', 'HeartRatePPG', 'StepCount']], chi_data_config, iter=300)
...
def cal_shuffle_dist(agg, sliding, num_point_per_day, realizd_df, fitbit_df, agg_daily_array,
                     model, cluster_list, min_array, max_array):
    ...
    for dates in range(dates_range):
        ...
        for shuffle_idx in range(100):
            ...
            agg_first_array = (agg_first_array[:, [0, 2, 3]] - min_array) / (max_array - min_array)
            agg_second_array = (agg_second_array[:, [0, 2, 3]] - min_array) / (max_array - min_array)
    
            first_cluster_array = model.predict(agg_first_array)
            second_cluster_array = model.predict(agg_second_array)
    
            first_distribution, second_distribution = np.zeros([48, len(cluster_list)]), np.zeros([48, len(cluster_list)])
    
            kl_dist_array = np.zeros([1, 48])
            for i in range(48):
                ...
                for cluster_id in cluster_list:
                    if cluster_id in cluster_list:
                        first_distribution[i, cluster_list.index(cluster_id)] = counter_dict[cluster_id]
    
                tmp_cluster_array = second_cluster_array[i * 30:(i + 1) * 30]
                counter_dict = Counter(tmp_cluster_array)
                for cluster_id in cluster_list:
                    if cluster_id in cluster_list:
                        second_distribution[i, cluster_list.index(cluster_id)] = counter_dict[cluster_id]
            ...
            dist_dict[dates][shuffle_idx]['first'] = first_cluster_array
            dist_dict[dates][shuffle_idx]['second'] = second_cluster_array
            dist_dict[dates][shuffle_idx]['dist'] = kl_dist_array
    
    return dist_dict


def cal_regular_dist(agg, sliding, num_point_per_day, realizd_df, fitbit_df,
                     agg_daily_array, model, cluster_list, min_array, max_array):
    dates_range = int(((pd.to_datetime(realizd_df.index[-1]) - pd.to_datetime(realizd_df.index[0])).days - agg) / sliding)
    ...
    for dates in range(dates_range):
        ...
        dist_dict[dates] = {}
        dist_dict[dates]['first'] = first_cluster_array
        dist_dict[dates]['second'] = second_cluster_array
        dist_dict[dates]['dist'] = kl_dist_array
...
def main(tiles_data_path, config_path, experiment):
    ...
    for idx, participant_id in enumerate(top_participant_id_list[8:]):
        ...
        # Clustering
        model, cluster_list, min_array, max_array = clustering_data(chi_data_config, agg, realizd_df, fitbit_df, agg_daily_array)
