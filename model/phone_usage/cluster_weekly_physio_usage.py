...
def cluster_data(data, data_config, iter=100):
    ...
    data_df = data.copy()
    data_df = data_df[['HeartRatePPG', 'StepCount']]
...
def main(tiles_data_path, config_path, experiment):
    ...
    for idx, participant_id in enumerate(top_participant_id_list[0:50]):
        ...
        for i in range(week_range):
            ...
            for j in range(7):
                ...
                time_list = [(start_time + timedelta(weeks=i, days=j) + timedelta(minutes=min * offset)).strftime(load_data_basic.date_time_format)[:-3] for min in range(num_point_per_day)]
                daily_df = pd.DataFrame(index=time_list, columns=['SecondsOnPhone', 'HeartRatePPG', 'StepCount'])
            ...
            week_dict['week_' + str(i)] = week_df

        final_data_df = cluster_data(final_data_df, chi_data_config, iter=300)
        
        for key_str in list(week_dict.keys()):
            time_list = list(week_dict[key_str].index)
            week_dict[key_str].loc[time_list, 'cluster'] = final_data_df.loc[time_list, 'cluster']

        output = open(os.path.join(chi_data_config.clustering_save_path, participant_id + '_physio.pkl'), 'wb')
