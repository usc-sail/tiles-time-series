...
icu_list = ['4 South', '5 North', '5 South ICU', '5 West', '7 West', '7 East', '7 South', '8 West',
			'4 south', '5 north', '5 south ICU', '5 west', '7 west', '7 east', '7 south', '8 west']
...
def main(tiles_data_path, config_path, experiment):
	...
	if os.path.exists(os.path.join('daily_realizd_fitbit_igtb.csv.gz')) is True:
		final_df = pd.read_csv(os.path.join('daily_realizd_fitbit_igtb.csv.gz'), index_col=0)
	else:
		...
		# Save the data
		final_df.to_csv('daily_realizd_fitbit_igtb.csv.gz', compression='gzip')
	...
	for compare_method in compare_method_list:
		...
		for col in igtb_cols + add_cols:
			stat_value, p_value = stats.ks_2samp(np.array(first_data_df[col].dropna()), np.array(second_data_df[col].dropna()))
			row_df[col] = p_value
		p_df = p_df.append(row_df)
		
		row_df = pd.DataFrame(index=[cond_list[0]])
		for col in igtb_cols + add_cols:
			mean = np.nanmean(first_data_df[col])
			mean = np.round(mean, 2)
			std = np.nanstd(first_data_df[col])
			std = np.round(std, 2)
			row_df[col] = str(mean) + ' (' + str(std) + ')'
		p_df = p_df.append(row_df)
		
		row_df = pd.DataFrame(index=[cond_list[1]])
		for col in igtb_cols + add_cols:
			mean = np.nanmean(second_data_df[col])
			mean = np.round(mean, 2)
			std = np.nanstd(second_data_df[col])
			std = np.round(std, 2)
			row_df[col] = str(mean) + ' (' + str(std) + ')'
		p_df = p_df.append(row_df)
