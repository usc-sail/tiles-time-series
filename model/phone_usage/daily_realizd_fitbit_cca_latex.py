...
def main(tiles_data_path, config_path, experiment):
	...
	for compare_method in compare_method_list:
		...
		for col in col_list:
			...
			# array1 = PCA(n_components=2).fit_transform(np.array(tmp_df[col[0]]))
			# array2 = PCA(n_components=2).fit_transform(np.array(tmp_df[col[1]]))
			...
			cca = rcca.CCA(kernelcca=False, reg=0.1, numCC=3) # linear
			...
			for i in range(len(score)):
				row_df[col[2] + '_p_val_' + str(i)] = test_score[i]
