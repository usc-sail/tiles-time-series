...
def main(tiles_data_path, config_path, experiment):
	...
	for idx, participant_id in enumerate(top_participant_id_list[:]):
		...
		if feat_type == 'ratio':
			...
			row_df['speak_rate_'+str(i)] = np.nan
		'''
		for i in range(num_of_sec):
			row_df['pitch_arousal_ratio_' + str(i)] = global_dict[i]['pitch_arousal_ratio']
			row_df['intensity_arousal_ratio_' + str(i)] = global_dict[i]['intensity_arousal_ratio']
	...
	for compare_method in compare_list:
		...
		for col in ana_igtb_cols_list:
			first_mr_df = first_data_df[[col] + feat_cols].dropna().copy()
			first_mr_df.loc[:, cond_list[0]] = 1
			first_mr_df.loc[:, cond_list[1]] = 0

			second_mr_df = second_data_df[[col] + feat_cols].dropna().copy()
			second_mr_df.loc[:, cond_list[0]] = 0
			second_mr_df.loc[:, cond_list[1]] = 1

			mr_df = first_mr_df.append(second_mr_df)

			# mr_df = first_data_df[[col] + feat_cols].dropna()
			y = mr_df[col]
			x = mr_df[feat_cols+cond_list]
			...
			p_row_df = pd.DataFrame(index=[compare_method + '_' + col])
			p_row_df['adj'] = model.rsquared_adj

			for index in list(model.pvalues.index):
				if 'const' in index or index in compare_list:
					continue
