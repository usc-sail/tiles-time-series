...
def main(tiles_data_path, config_path, experiment):
    ...
    for idx, participant_id in enumerate(top_participant_id_list):
        ...
        if os.path.exists(os.path.join(data_cluster_path, participant_id, save_prefix + '_corr.csv.gz')) is True:
            topic_corr_df = pd.read_csv(os.path.join(data_cluster_path, participant_id, save_prefix + '_corr.csv.gz'), index_col=0)
            ...
            for col in col_list:
                ...
                # participant_corr_df[col] += int(len(np.where(np.array(topic_corr_df.loc[:, col]) < -0.2)[0]))
                participant_corr_df[col] = 1 if int(len(np.where(np.array(topic_corr_df.loc[:, col]) > 0.2)[0])) > 0 else 0
                participant_corr_df[col] = 1 if int(len(np.where(np.array(topic_corr_df.loc[:, col]) < -0.2)[0])) > 0 else participant_corr_df[col]
