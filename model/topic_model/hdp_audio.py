...
def main(tiles_data_path, config_path, experiment):
	...
	for idx, participant_id in enumerate(top_participant_id_list):
		...
		if os.path.exists(os.path.join(data_config.audio_sensor_dict['clustering_path'], participant_id, 'lda_utterance_cluster.csv.gz')) is False:
			continue

		# file_list = [file for file in os.listdir(os.path.join(data_config.audio_sensor_dict['clustering_path'], participant_id))]
		# for file in file_list:

		data_df = pd.read_csv(os.path.join(data_config.audio_sensor_dict['clustering_path'], participant_id, 'lda_utterance_cluster.csv.gz'), index_col=0)
		...
		hdp = HdpModel(word_corpus, word_dictionary, T=3)
